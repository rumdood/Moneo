...
public class CreateOrUpdateTaskWorkflowManager : ITaskWorkflowManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (response is null)
        {
            switch (stateMachine.CurrentState)
            {
                ...
                case TaskCreateOrUpdateState.WaitingForRepeaterCron:
                    // Send CreateCronRequest directly
                    return await _sender.Send(new CreateCronRequest(context.ConversationId, context.User, context.CurrentState), cancellationToken);
            }
        ...
        switch (stateMachine.CurrentState)
        {
            ...
            case TaskCreateOrUpdateState.WaitingForRepeaterCron:
                // Send CreateCronRequest directly
                return await _sender.Send(new CreateCronRequest(context.ConversationId, null, context.CurrentState), cancellationToken);
        }
